...
class Functions():
    ...
    def zoom_graph(equation,lower_limit,upper_limit):
        ...
        print('xii')
        session_id=str(id(session))
        file_str='app//temp//zoom_graph'+session_id+'.gif'
        ani.save(file_str,writer=writer)
        with open(file_str, "rb") as image_file:
            data = base64.b64encode(image_file.read()).decode()
            print('zoomer: ',type(data))
        os.remove(file_str)
    ...
    def tang_graph_start(megalist_x2,megalist_y2,tang_starts2,equation,mask_starts):
        ...
        writer=animation.PillowWriter(fps=15,metadata=dict(artist='Me'),bitrate=1800)
        session_id=str(id(session))
        file_str='app//temp//tang_start_graph'+session_id+'.gif'
        ani.save(file_str,writer=writer)
        with open(file_str, "rb") as image_file:
            data = base64.b64encode(image_file.read()).decode()
    ...
    def tang_graph_end(megalist_x3,megalist_y3,tang_starts2,equation,mask_starts):
        ...
        writer=animation.PillowWriter(fps=15,metadata=dict(artist='Me'),bitrate=1800)
        session_id=str(id(session))
        file_str='app//temp//tang_end_graph'+session_id+'.gif'
        ani.save(file_str,writer=writer)
        with open(file_str, "rb") as image_file:
            data = base64.b64encode(image_file.read()).decode()
            print('end tan: ',type(data))
        os.remove(file_str)
        return data
